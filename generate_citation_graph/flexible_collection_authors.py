...
class FlexibleCitationParser:
    def __init__(self, author_records_path='data/teacher_graph/records/author_records.pkl'):
        self.batch_size = 100
        self.author_records_path = author_records_path
        self.author_records = pickle.load(open(self.author_records_path, "rb"))
    ...
    def save_processed_ids(self):
        pickle.dump(self.author_records, open(self.author_records_path, "wb"))
    ...
    def collect_citations_batch(self, s2_ids, fields=None):
        ...
        for paper_idx in tqdm(range(0, len(s2_ids), self.batch_size), total=len(s2_ids)//self.batch_size):
            ...
            for i, paper_id in enumerate(s2_ids[paper_idx:paper_idx + self.batch_size]):
                ...
                record = {"authorId": author['authorId'],
                    "name": author['name'],
                ...
                self.author_records[paper_id] = record
...
if __name__ == "__main__":
    ...
    # load the ids
    remaining_author_ids = list(pickle.load(open('data/teacher_graph/remaining_author_ids.pickle', "rb")))

    citation_parser = FlexibleCitationParser()
    fields = "authorId,name,affiliations,paperCount,citationCount,hIndex"
    citation_parser.collect_citations_batch(s2_ids=remaining_author_ids, fields=fields)   
